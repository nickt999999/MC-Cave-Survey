...
For making a cave map, you'll want a cave survey compass, survey data/graph pages, and a pencil. The survey compass is a small plastic piece with a short ruler and azimuth dial useful for sketching point-to-point lines with the right horizontal distance and azimuth angle. As of 2025, they're sold for $11.00 + shipping by Final Frontier Sports (https://www.finalfrontiersports.com/product/cave-compass-standard). Survey pages are also sold on waterproof paper by cave gear retailers, but for the datapack, printing or annotating a pdf works well. Downloadable pdf files for survey data and graph pages are available on Derek Bristol's website (https://www.derekbristol.com/survey-efficiency).
<br>
<br>
Aside from taking measurement shots and sketching features in Minecraft, the mapping/sketching process mirrors the cave survey process that cavers use to build maps of real-world caves! There's some good resources online for learning the cave survey process — Derek Bristol, the Out of Bounds Grotto, and cavediggers.com have some good guides/info on this. Before taking in-game Disto shots, it's a good idea to mark each point you're shooting between with a Wite-Out blot and label it with flagging tape. Taking each shot from the nearest Wite-Out blot position gives the best results — if the starting position is the player's eyes instead, reposition the Wite-Out blot so there's a clear line of sight between each point and try again. You can use the Disto to take forwards shots, backsight shots (backwards direction shots to double-check forwards measurements), and LRUD shots (left/right/up/down shots noting the distance to each cave wall) at each survey point for sketching. You can take all these shots and sketch the cave yourself, or you can divide up the roles for speed.
