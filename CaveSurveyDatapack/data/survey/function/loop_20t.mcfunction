...
execute as @a at @s at @e[tag=survey.witeout.blot_target_pos,type=marker,distance=..300] if block ^ ^ ^0.01 #survey:item_raycast_transparent run function survey:witeout/break
execute as @a at @s at @e[tag=survey.flagging_tape.piece_target_pos,type=marker,distance=..300] if block ^ ^ ^0.001 #survey:item_raycast_transparent run function survey:flagging_tape/use/break
