...
execute positioned ~0.0001 ~0.0001 ~0.0001 as @e[dx=0,tag=!survey.self,type=!marker,distance=..30] positioned ~-1 ~-1 ~-1 if entity @s[dx=0,distance=..30] run return run function survey:witeout/raycast/end {raycast_result:"entity"}
# end raycast if range limit reached
execute if score #survey survey.witeout.raycast_steps matches 500 run return run function survey:witeout/raycast/end {raycast_result:"range_limit"}
# increment raycast step counter
scoreboard players add #survey survey.witeout.raycast_steps 1
