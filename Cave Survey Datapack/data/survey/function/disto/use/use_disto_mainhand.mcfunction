# reset advancement
advancement revoke @s only survey:use_disto_mainhand

# prevent if within cooldown
execute if score @s disto_cooldown_gametime matches 0.. run return 0

# else - take disto shot
data modify storage survey:data disto.shot.slot set value "mainhand"
data modify storage survey:data disto.shot.disto_id set from entity @s SelectedItem.components."minecraft:custom_data"."disto_id"
