...
execute store result score #survey survey.general.max_command_sequence_length run gamerule max_command_sequence_length
...
execute store result score #survey survey.general.last_gametime run data get storage survey:data general.gametime
scoreboard players operation #survey survey.general.gametime_change = #survey survey.general.current_gametime
scoreboard players operation #survey survey.general.gametime_change -= #survey survey.general.last_gametime
execute if data storage survey:data general.gametime if score #survey survey.general.gametime_change matches 100.. run tellraw @a [{"text":"["},{"text":"SURVEY","color":"red"},{"text":"] "},{"text":"Cave survey datapack enabled. Use "},{"text":"/trigger survey.help","color":"yellow","click_event":{"action":"suggest_command","command":"/trigger survey.help"}},{"text":" for info & datapack commands"}]
execute if data storage survey:data general.gametime if score #survey survey.general.gametime_change matches 100.. unless score #survey survey.general.max_command_sequence_length matches 65536.. run tellraw @a [{"text":"["},{"text":"SURVEY","color":"red"},{"text":"] Low max_command_sequence_length value detected - this may interfere with the cave survey datapack. It's reccomended to reset max_command_sequence_length to its default value using "},{"text":"/gamerule max_command_sequence_length 65536","color":"yellow","click_event":{"action":"suggest_command","command":"/gamerule max_command_sequence_length 65536"}}]
...
execute as @a if score @s survey.general.uninstall_timer matches 1..300 run scoreboard players remove @s survey.general.uninstall_timer 20
