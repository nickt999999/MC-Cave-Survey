...
execute if score @s survey.general.click_writable_book matches 1.. run function survey:flagging_tape/use/raycast
execute unless score @s survey.general.click_writable_book matches 1.. if function survey:flagging_tape/text_check/if_text_written run function survey:flagging_tape/use/raycast

# break flagging tape if player right click item and raycast hits flagging tape
execute if score @s survey.general.click_writable_book matches 1.. at @e[tag=flagging_tape_target_pos,type=marker,distance=..10] run function survey:flagging_tape/use/break
